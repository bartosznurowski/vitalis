...
class CatalogLogic implements EntityLogicInterface
{
    ...
    {
        return $formFactory->create(CatalogType::class, $entityInstance);
    }

    public function validateNewEntityForm(object $entityInstance, FormInterface $form): void
    {

        if (!$entityInstance instanceof Catalog) {
        ...
        $directoryPath = $this->parameters->get('kernel.project_dir') . '/var/files/nutrition_entity_catalog/' . $entityInstance->getId();
